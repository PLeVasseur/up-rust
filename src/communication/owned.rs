...
fn build_message_with_payload<S: crate::umessage::BuilderState>(
    ...
    match payload {
        Some(payload) => {
            builder.build_with_payload(payload.payload().clone(), payload.payload_encoding())
        }
        None => builder.build(),
    }
}
        ...
        match response.commstatus() {
            Some(UCode::Ok) | None => {
                let response_payload = match (response.payload(), response.payload_encoding()) {
                    ...
                    // A payload without a declared encoding is not consumable; the
                    // legacy path wrapped it as Unspecified and every downstream
                    // extraction rejected it, so surfacing no payload is equivalent
                    // and honest under the unified mechanism.
                    (Some(_), None) => None,
                    (None, _) => None,
                };
                Ok(response_payload)
        ...
        T::Wire: UWireEncode<Request> + UWireDecodeOwned<Response>,
    {
        ...
        let response = self
            .invoke_method(
                method,
                call_options,
                Some(UPayload::new(payload_bytes, payload_encoding)),
            )
            .await?
...
mod tests {
    ...
    use super::*;
    use crate::{
        PayloadEncoding, StaticUriProvider, UAttributes, UCode, UOwnedFrame, UOwnedTransportImpl,
        UStatus,
    };
    ...
    async fn subscriber_registers_owned_listener_and_delivers_messages() {
        ...
        let message = UMessageBuilder::publish(topic.clone())
            .build_with_payload("event", PayloadEncoding::TEXT)
    ...
    async fn notifier_sends_and_listens_with_owned_frames() {
        ...
        let notification =
            UMessageBuilder::notification(topic.clone(), uri_provider().get_source_uri())
                .build_with_payload("notification", PayloadEncoding::TEXT)
        ...
        notifier
            .notify(
                ...
                Some(UPayload::new("notify", PayloadEncoding::TEXT)),
            )
            .await
    ...
    async fn rpc_client_sends_request_and_returns_response_payload() {
        ...
        let request =
            UMessageBuilder::request(method_uri(), uri_provider().get_source_uri(), 5_000)
                ...
                .build_with_payload("request", PayloadEncoding::TEXT)
                .expect("request");
        let response = UMessageBuilder::response_for_request(request.attributes())
            .build_with_payload("response", PayloadEncoding::TEXT)
            .expect("response");
        ...
        let result = client
            .invoke_method(
                ...
                Some(UPayload::new("request", PayloadEncoding::TEXT)),
            )
            .await
            .expect("rpc succeeds")
    ...
    async fn rpc_server_registers_listener_and_sends_response() {
        ...
        let request =
            UMessageBuilder::request(method_uri(), uri_provider().get_source_uri(), 5_000)
                .build_with_payload("server request", PayloadEncoding::TEXT)
                .expect("request");
...
mod selected_wire_tests {
    ...
    use super::*;
    use crate::{
        NativePrefixFrameMetadataCodec, ProtobufWire, UHasWire, UOwnedTransportCore, UStatus,
