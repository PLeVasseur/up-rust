...
use crate::{
    ...
    zero_copy::{UTxBuffer, UUninitTxBuffer, UVecUninitTxBuffer},
    PayloadEncoding, UFrameMetadata, UMessageType, UUri, UWireError,
};
...
pub fn init_camera_8mp_bayer_rggb12p(
    ...
    init.header(|header| init_header(header, case, sequence))?
        ...
        .analog_gain(1.5)
        .digital_gain(1.0)
        .intrinsics(|intrinsics| init_camera_intrinsics(intrinsics))?
        .extrinsics(|pose| init_pose(pose, sequence))?
        .roi(|roi| {
            roi.x(0)
                .y(0)
    ...
    T: StablePayload + StablePayloadInit,
{
    ...
    let metadata = fixture_metadata().with_encoding(encoding.clone());
    let mut buffer =
        UVecUninitTxBuffer::with_alignment(metadata, mem::size_of::<T>(), mem::align_of::<T>())?;
    let initializer = T::init_from_uninit_payload(buffer.payload_uninit_mut())?;
    init(initializer)?;
...
fn fixture_metadata() -> UFrameMetadata {
    UFrameMetadata::new_unchecked(
        crate::UAttributes::new_unchecked(
            crate::UUID::build(),
