...
Build semantic message metadata and payload explicitly at the transport layer:
...
let message = UMessageBuilder::publish(topic)
    .build_with_payload("92.5", PayloadEncoding::TEXT)?;
assert_eq!(message.payload().as_deref(), Some(&b"92.5"[..]));
# Ok::<(), Box<dyn std::error::Error>>(())
```

Applications that prefer role-oriented publish, notify, subscribe, and RPC APIs
should start with the [guide](crate::guide::applications).

## Which reader are you?

* Application authors: [writing an application](crate::guide::applications).
* Communication-role users: [the up-L2 guide](crate::guide::applications::communication).
* Transport users: [the transport-layer guide](crate::guide::applications::transport).
* Transport implementers: [transport implementation](crate::guide::utransport).
* Wire implementers: [selected wires](crate::guide::wires) and the
  [trait map](crate::guide::trait_map).
...
pub use payload::codec::{
    ...
    PayloadLayout, ProtobufPayload, ReadDecodePayload,
};
#[cfg(feature = "zero-copy-transport")]
pub use payload::loan::BorrowPayload;
#[cfg(feature = "zero-copy-transport")]
pub use payload::stable::{
    stable_payload_encoding_id, stable_payload_encoding_id_for, InitializedStablePayload,
    StableContainerPayload, StableMemoryRepresentation, StablePayload, StablePayloadField,
