...
use crate::{
    payload::{
        BytePayloadCodec, EncodePayload, EncodedPayload, InitializedStablePayload, LoanPayload,
        LoanUninitPayload, LoanedInitPayload, PayloadCodec, PayloadFormat, PayloadLayout,
        StableContainerPayload, StablePayloadInit, USerializer, UWireError,
    },
    zero_copy::{
...
use crate::payload::{StablePayload, UnsafeStablePayloadTxSlot};
...
pub trait UZeroCopyUninitTransportExt: UZeroCopyUninitTransport {
    ...
    /// fields and generated padding gaps are initialized. This path reserves the
    /// exact `StableContainerPayload<T>` layout and does not call the older
    /// `LoanPayload` helper that creates a `&mut T` by first zero-initializing the
    /// whole payload.
        ...
        T: StablePayloadInit + Send,
    {
        ...
        verify_reserved_uninit_payload_layout(&mut buffer, layout)?;
        {
            ...
            let initializer = T::init_from_uninit_payload(payload).map_err(UStatus::from)?;
            let _initialized = init(initializer).map_err(UStatus::from)?;
        }
        // SAFETY:
        // - `StablePayloadInit::init_from_uninit_payload` validated the exact
        //   stable layout before creating the generated builder.
        // - The closure can return only an initialized marker produced by the
        //   builder's all-fields-set `finish()` or an expert manual implementation
        //   of the unsafe trait.
        let buffer = unsafe { buffer.assume_payload_init() };
        self.send_zero_copy(buffer).await
    }
