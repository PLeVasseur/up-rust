...
impl PayloadEncoding {
    ...
    /// Declares a structurally valid identifier in a constant context.
    ///
    /// The caller is responsible for its registry or private-contract meaning;
    /// this constructor checks the range, not registration or decoder support.
    ///
    /// # Panics
    /// Panics when `id` is greater than `65535`.
    #[must_use]
    pub const fn from_registry_entry(id: u32) -> Self {
        assert!(
            id <= u16::MAX as u32,
            "payload-encoding identifier exceeds 65535"
        );
        Self(id as u16)
    }
