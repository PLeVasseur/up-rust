...
pub trait UZeroCopyTransportExt: UZeroCopyTransport {
    ...
    ///
    /// Prefer this selected-wire helper on values produced by `.with_wire(W)`.
    ...
    /// `.with_wire(W)` should prefer [`Self::send_loaned_payload`] so the
...
pub trait UZeroCopyUninitTransportExt: UZeroCopyUninitTransport {
    ...
    ///
    /// This is the low-level codec-selected form. Product code that already uses
    /// `.with_wire(W)` should prefer [`Self::send_uninit_loaned_payload`] so the
    /// selected wire supplies the payload codec.
    ///
    /// # Errors
    ///
    ...
    /// uses `.with_wire(W)` should prefer [`Self::send_uninit_stable_payload`] so
