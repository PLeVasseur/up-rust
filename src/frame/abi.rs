...
pub const UFRAME_ABI_SIZE: usize = 928;
...
/// Fixed-layout payload encoding. size 4, align 4, no padding.
///
/// The payload encoding as its registry entry identifier.
///
/// `0` when absent (the identifier is reserved by the registry, so zero is
/// unambiguous); presence is additionally tracked by the frame ABI's field
/// presence bits.
#[repr(C)]
#[derive(Clone, Copy, Debug, Default, Eq, PartialEq)]
pub struct UPayloadEncodingAbi {
    /// Payload-encoding registry entry identifier; `0` when absent.
    pub registry_id: u32,
}
...
///
/// size 928, align 8, no implicit padding. A field guarded by a `FIELD_*`
...
impl UFrameMetadataAbiV1 {
    ...
    pub fn try_from_metadata(
        ...
        if let Some(encoding) = metadata.payload_encoding() {
            abi.payload_encoding.registry_id = encoding.id();
    ...
    pub fn try_to_metadata(&self) -> Result<(UFrameMetadata, Option<u64>), UFrameAbiError> {
        ...
        let (payload_encoding, payload_size) = if self.presence & FIELD_PAYLOAD_ENCODING != 0 {
            (
                Some(
                    PayloadEncoding::from_id(self.payload_encoding.registry_id).map_err(|_| {
                        UFrameAbiError::InvalidProfile(
                            "payload-encoding identifier 0 with presence bit set".to_string(),
                        )
                    })?,
                ),
...
const _: () = {
    ...
    assert!(offset_of!(UFrameMetadataAbiV1, source) == 76);
    assert!(offset_of!(UFrameMetadataAbiV1, sink) == 212);
    assert!(offset_of!(UFrameMetadataAbiV1, traceparent) == 348);
    assert!(offset_of!(UFrameMetadataAbiV1, token) == 412);
    assert!(offset_of!(UFrameMetadataAbiV1, _reserved_tail) == 924);

    assert!(offset_of!(UPayloadEncodingAbi, registry_id) == 0);
    assert!(core::mem::size_of::<UPayloadEncodingAbi>() == 4);
};
