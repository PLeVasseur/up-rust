...
inline constexpr std::size_t UFRAME_ABI_SIZE = 928;
...
struct UPayloadEncodingAbi {
    std::uint32_t registry_id; // valid iff FIELD_PAYLOAD_ENCODING is set
};
...
static_assert(sizeof(UPayloadEncodingAbi) == 4 && alignof(UPayloadEncodingAbi) == 4);
...
static_assert(offsetof(UFrameMetadataAbiV1, source) == 76);
static_assert(offsetof(UFrameMetadataAbiV1, sink) == 212);
static_assert(offsetof(UFrameMetadataAbiV1, traceparent) == 348);
static_assert(offsetof(UFrameMetadataAbiV1, token) == 412);
static_assert(offsetof(UFrameMetadataAbiV1, reserved_tail) == 924);
