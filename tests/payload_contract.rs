...
use up_rust::{
    bench_fixtures::payload_contract::{self as fixtures, *},
    payload::StablePayload,
};

#[cfg(feature = "test-util")]
use up_rust::{
    test_util::InMemoryZeroCopyTransport,
    zero_copy::{
        ULoanedContiguousZeroCopyRxFrame, UZeroCopyTransport, UZeroCopyUninitTransportExt,
    },
    UFrameMetadata, UUri,
};
...
fn payload_contract_protobuf_fixtures_round_trip_and_validate() {
    for case in fixtures::all_cases() {
        ...
        fixtures::validate_protobuf_bytes(case, 11, &bytes).expect(case.name());
    }
}
...
fn payload_contract_stable_owned_fixtures_validate_representative_and_exact_bytes() {
    for case in fixtures::all_cases() {
        let fixture = fixtures::stable_owned_fixture_for(case, 17).expect(case.name());
        assert_eq!(
...
#[test]
fn payload_contract_default_representative_fixtures_are_not_byte_backed() {
    assert!(!CanClassicFrameV1::SUPPORTS_BYTE_BACKED_UNINIT);
    assert!(!SomeIpSignalBatchMtuV1::SUPPORTS_BYTE_BACKED_UNINIT);
    assert!(!RadarDetectionListArs548V1::SUPPORTS_BYTE_BACKED_UNINIT);
}
